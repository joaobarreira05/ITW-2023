var vm = function () {
    ...
    //self.baseUri = ko.observable('http://localhost:62595/api/players');
    ...
    self.favoritePlayer = function (id, event) {
        ...
        if (JSON.parse(window.localStorage.getItem('favPlayers0')) == null) {
            console.log('no favPlayers in local storage');
            ...
            for (var i = 0; i < c.length; i++) {
                if (id == c[i]) {
                    c.splice(i, 1);
                    window.localStorage.setItem('favPlayers0', JSON.stringify(c));
    ...
    $("#searchbar").autocomplete({
        ...
        select: function (e, ui) {
            $.ajax({
                ...
                success: function (data) {
                    window.location = 'playerdetails.html?id=' + data[0].Id;
                }
            })
        },
        messages: {
